...
<button class="relative flex h-full w-full select-none flex-col items-center justify-center">
  ...
  <div class="scrambleContainer absolute w-full h-[20%] md:top-4 top-20 md:px-28 px-4  block ">
    {#if $scramble.length > 200}
    
    <p class="scramble text-[3.3cqmin] text-balance" >
      {$scramble}
    </p>
    ...
    {:else}
    <p class="scramble text-[7cqmin] text-balance" >
      {$scramble}
    </p>
    {/if}
